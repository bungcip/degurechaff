...
function dumpPairs(node: { [index: string]: any }, pairs: ast.Pair[]) {
...
function toValueSpec(data: ast.Value): any {
...
function testSpec(folder: string, filename: string) {
...
function testInvalid(folder: string, filename: string) {
  const tomlData = fs.readFileSync(`${folder}/${filename}.toml`, 'utf8')
